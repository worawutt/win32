...
typedef CACHE_DESTROY_CALLBACK = Void Function(Uint32 cb, Pointer<Uint8> lpb);
typedef CACHE_KEY_COMPARE = Int32 Function(Uint32 cbKey1,
    Pointer<Uint8> lpbKey1, Uint32 cbKey2, Pointer<Uint8> lpbKey2);
typedef CACHE_KEY_HASH = Uint32 Function(Pointer<Uint8> lpbKey, Uint32 cbKey);
typedef CACHE_READ_CALLBACK = Int32 Function(
    Uint32 cb, Pointer<Uint8> lpb, Pointer lpvContext);
typedef CLAIMMEDIALABEL = Uint32 Function(Pointer<Uint8> pBuffer,
    Uint32 nBufferSize, Pointer<MediaLabelInfo> pLabelInfo);
...
typedef CLFS_BLOCK_ALLOCATION = Pointer Function(
    Uint32 cbBufferLength, Pointer pvUserContext);
typedef CLFS_BLOCK_DEALLOCATION = Void Function(
    Pointer pvBuffer, Pointer pvUserContext);
...
typedef LPPROGRESS_ROUTINE = Uint32 Function(
    ...
    Pointer lpData);
typedef MAXMEDIALABEL = Uint32 Function(Pointer<Uint32> pMaxSize);
typedef PCLFS_COMPLETION_ROUTINE = Void Function(
    Pointer pvOverlapped, Uint32 ulReserved);
typedef PCOPYFILE2_PROGRESS_ROUTINE = Int32 Function(
    Pointer<COPYFILE2_MESSAGE> pMessage, Pointer pvCallbackContext);
typedef PFE_EXPORT_FUNC = Uint32 Function(
    Pointer<Uint8> pbData, Pointer pvCallbackContext, Uint32 ulLength);
typedef PFE_IMPORT_FUNC = Uint32 Function(
    Pointer<Uint8> pbData, Pointer pvCallbackContext, Pointer<Uint32> ulLength);
typedef PFN_IO_COMPLETION = Void Function(Pointer<FIO_CONTEXT> pContext,
    Pointer<FH_OVERLAPPED> lpo, Uint32 cb, Uint32 dwCompletionStatus);
typedef PLOG_FULL_HANDLER_CALLBACK = Void Function(IntPtr hLogFile,
    Uint32 dwError, Int32 fLogIsPinned, Pointer pvClientContext);
typedef PLOG_TAIL_ADVANCE_CALLBACK = Void Function(
    IntPtr hLogFile, CLS_LSN lsnTarget, Pointer pvClientContext);
typedef PLOG_UNPINNED_CALLBACK = Void Function(
    IntPtr hLogFile, Pointer pvClientContext);
typedef WofEnumEntryProc = Int32 Function(Pointer EntryInfo, Pointer UserData);
typedef WofEnumFilesProc = Int32 Function(
    Pointer<Utf16> FilePath, Pointer ExternalFileInfo, Pointer UserData);
