// IAMCopyCaptureFileProgress.dart

// THIS FILE IS GENERATED AUTOMATICALLY AND SHOULD NOT BE EDITED DIRECTLY.

// ignore_for_file: unused_import, directives_ordering

import 'dart:ffi';

import 'package:ffi/ffi.dart';

import '../../combase.dart';
import '../../constants.dart';
import '../../exceptions.dart';
import '../../guid.dart';
import '../../macros.dart';
import '../../ole32.dart';
import '../../utils.dart';

import '../../system/com/IUnknown.dart';
import '../../foundation/structs.g.dart';

/// @nodoc
const IID_IAMCopyCaptureFileProgress = '{670D1D20-A068-11D0-B3F0-00AA003761C5}';

/// {@category Interface}
/// {@category com}
class IAMCopyCaptureFileProgress extends IUnknown {
  // vtable begins at 3, is 1 entries long.
  IAMCopyCaptureFileProgress(Pointer<COMObject> ptr) : super(ptr);

  int Progress(int iProgress) => ptr.ref.lpVtbl.value
      .elementAt(3)
      .cast<Pointer<NativeFunction<Int32 Function(Pointer, Int32 iProgress)>>>()
      .value
      .asFunction<
          int Function(Pointer, int iProgress)>()(ptr.ref.lpVtbl, iProgress);
}
