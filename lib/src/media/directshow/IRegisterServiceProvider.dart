// IRegisterServiceProvider.dart

// THIS FILE IS GENERATED AUTOMATICALLY AND SHOULD NOT BE EDITED DIRECTLY.

// ignore_for_file: unused_import, directives_ordering

import 'dart:ffi';

import 'package:ffi/ffi.dart';

import '../../combase.dart';
import '../../constants.dart';
import '../../exceptions.dart';
import '../../guid.dart';
import '../../macros.dart';
import '../../ole32.dart';
import '../../utils.dart';

import '../../system/com/IUnknown.dart';
import '../../specialTypes.dart';
import '../../foundation/structs.g.dart';

/// @nodoc
const IID_IRegisterServiceProvider = '{7B3A2F01-0751-48DD-B556-004785171C54}';

/// {@category Interface}
/// {@category com}
class IRegisterServiceProvider extends IUnknown {
  // vtable begins at 3, is 1 entries long.
  IRegisterServiceProvider(Pointer<COMObject> ptr) : super(ptr);

  int RegisterService(
          Pointer<GUID> guidService, Pointer<COMObject> pUnkObject) =>
      ptr.ref.lpVtbl.value
              .elementAt(3)
              .cast<
                  Pointer<
                      NativeFunction<
                          Int32 Function(Pointer, Pointer<GUID> guidService,
                              Pointer<COMObject> pUnkObject)>>>()
              .value
              .asFunction<
                  int Function(Pointer, Pointer<GUID> guidService,
                      Pointer<COMObject> pUnkObject)>()(
          ptr.ref.lpVtbl, guidService, pUnkObject);
}
